...
const ProductCard = ({ locale, type = 'our', typePage, product }: ProductCardProps) => {
    // 1. ЗАХИСТ: Якщо продукту немає, не рендеримо нічого
    ...
    // 2. БЕЗПЕЧНІ ДАНІ
    // Беремо title для поточної локалі, або українську, або пустий рядок
    const productTitle = product.title?.[locale] || product.title?.['uk'] || '';
    
    // Визначаємо слаг безпечно
    const slug = locale === 'en' 
        ? (product.slugEn || product.slugUk) 
        : (product.slugUk || product.slugEn);
        
    const productUrl = slug ? `/product/${slug}` : '#';

    // Картинка: перевіряємо, чи є масив і чи є в ньому елементи
    const productImage = (product.images && product.images.length > 0) ? product.images[0] : '';

    return (
        <article className={`${baseWidth} rounded-md border-2 border-sc-1 flex flex-col justify-between items-center relative product-card-shadow`}>
