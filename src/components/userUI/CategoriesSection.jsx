          ...
          <div key={index} className='flex flex-col pt-2 py-8 pb-8 bg-bg-light'>
            ...
            <div className='flex flex-col items-center'>
              ...
              <div className='min-h-16 flex justify-center items-center'>
                <h3 className={`px-4 text-center font-semibold text-xl mx-auto  bg-clip-text text-transparent ${categories.length === index+1 ? 'bg-bronze' : 'bg-primary'}`}>
                  {item[lang]}
                </h3>
              </div>
            </div>

            <div className={`h-0.5 w-10/12 mb-2 mx-auto ${categories.length === index+1 ? 'bg-bronze' : 'bg-primary'}`}></div>

            <ul className='w-10/12 mx-auto'>
              {subcategories.map((item, index) => {
                if(index > 6) return
                return (
                  <li key={index} className='text-xl mb-4'>{item}</li>
                )
              })}
            </ul>

            <div className='mx-auto'>
              <Link href={`/category`} className={`text-center font-semibold ${categories.length === index+1 ? 'bronze-text' : 'gradient-text'}`}>
