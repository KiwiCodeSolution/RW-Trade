...
const CardStateLabel = ({ variant = 'green', size = 's', className = '' }) => {
	const baseStyle =
		'flex justify-center items-center rounded-lg hover:shadow-lg hover:scale-102 duration-300'
	...
	return <button className={combined}>{variant === 'yellow' ? <Clock /> : <CartPlus />}</button>
}
